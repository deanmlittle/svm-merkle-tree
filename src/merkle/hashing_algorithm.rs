...
use alloc::vec::Vec;
#[cfg(target_arch = "wasm32")]
use hashing_wasm::{keccak256, sha256};
...
mod hashing {
    pub fn sha256(val: &[u8]) -> [u8; 32] {
        solana_sha256_hasher::hash(val).to_bytes()
    }

    pub fn keccak256(val: &[u8]) -> [u8; 32] {
        solana_keccak_hasher::hash(val).to_bytes()
    }
}
#[cfg(not(target_arch = "wasm32"))]
use hashing::{keccak256, sha256};
...
impl HashingAlgorithm {
    ...
    pub fn double_hash(&self, b: &[u8], s: usize) -> Vec<u8> {
        ...
        match self {
            HashingAlgorithm::Sha256 | HashingAlgorithm::Sha256d => {
                sha256(&sha256(b))[..s].to_vec()
            }
            HashingAlgorithm::Keccak | HashingAlgorithm::Keccakd => {
                keccak256(&keccak256(b))[..s].to_vec()
            }
        }
    }
}
